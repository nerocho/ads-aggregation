<template>
  ...
  >
    ...
    <p class="mt-2 text-center text-black/50 dark:text-slate-500">
      {{ hero.subTitle }}
    </p>
...
import { data } from '../utils/article.data.js';
...
const categories =
  types ||
  sidebarData?.items.map((item: any) => ({ name: item.text, link: item.link }));
...
let posts = data;
if (category !== 'all') {
  posts = data.filter((post) => post.url.startsWith(`/Notes/${category}/`));
}
</script>
